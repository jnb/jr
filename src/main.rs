...
struct App<J: JujutsuOps, G: GitOps, H: GithubOps> {
    jj: J,
    git: G,
    gh: H,
}
...
impl<J: JujutsuOps, G: GitOps, H: GithubOps> App<J, G, H> {
    fn new(jj: J, git: G, gh: H) -> Self {
        Self { jj, git, gh }
    }

    async fn cmd_create(&self, revision: &str) -> Result<()> {
        ...
        println!("Updated PR branch {}", pr_branch);

        // Push PR branch
        self.git.push_branch(&pr_branch)?;
        println!("Pushed PR branch {}", pr_branch);
        ...
        let pr_url = self
            ...
            .await?;
        println!("Created PR for {} with base {}", pr_branch, base_branch);
        println!("PR URL: {}", pr_url);

        Ok(())
    }

    async fn cmd_update(&self, revision: &str, message: Option<&str>) -> Result<()> {
        ...
        println!("PR branch {} exists", pr_branch);
        ...
        let commit_message = if let Some(msg) = message {
            ...
            if local_change_diff == pr_cumulative_diff {
                ...
                println!("Detected pure restack (no changes to this commit)");
                "Restack".to_string()
            } else {
                return Err(anyhow::anyhow!(
        ...
        let new_commit = if tree == old_pr_tree && !base_has_changed {
            println!("Tree unchanged and base hasn't moved, reusing old PR tip commit");
            old_pr_tip.clone()
        } else {
            // Create merge commit with old PR tip and base as parents
            let commit = self.git.commit_tree_merge(&tree, &[&old_pr_tip, &base_tip], &commit_message)?;
            println!("Created new merge commit: {}", commit);
            commit
        };

        // Only update if there are actual changes
        if new_commit == old_pr_tip {
            println!("No changes to push - PR is already up to date");
            return Ok(());
        }
        ...
        let pr_url = if self.gh.pr_is_open(&pr_branch).await? {
            ...
            println!("Updated PR for {} with base {}", pr_branch, base_branch);
            url
        } else {
            return Err(anyhow::anyhow!(
    ...
    async fn cmd_status(&self) -> Result<()> {
        ...
        let changes: Vec<(String, String)> = if heads.is_empty() {
            ...
            eprintln!("Warning: Multiple stack heads detected. Showing stack from @ to trunk.");
        ...
        // Prepare tasks to fetch PR URLs and base branches concurrently
                                ...
                                let base_has_moved = if let Some(base_branch) = _base_branch {
                                    ...
                                    if let Ok(pr_branch_tip) = self.git.get_branch(expected_branch) {
                                        ...
                                        if let Ok(base_tip) = self.git.get_branch(base_branch) {
                                            ...
                                            !self.git.is_ancestor(&base_tip, &pr_branch_tip).unwrap_or(false)
                                        } else {
                                            false
                                        }
                                    } else {
    ...
    async fn check_parent_prs_up_to_date(&self, revision: &str) -> Result<()> {
        ...
        let branches_to_check: Vec<_> = stack_changes
            ...
            .filter_map(|(change_id, _)| {
                ...
                let expected_branch = format!("{}{}", GLOBAL_BRANCH_PREFIX, short_change_id);
                if all_branches.contains(&expected_branch) {
                    Some((change_id.clone(), expected_branch))
                } else {
                    None
                }
            })
        ...
        for ((change_id, expected_branch), (_, pr_diff_result)) in
            branches_to_check.iter().zip(pr_diff_results.iter())
        {
            ...
            let pr_diff = pr_diff_result.as_ref().map_err(|e| anyhow::anyhow!("{}", e))?;
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Some(Commands::Create { revision }) => app.cmd_create(&revision).await?,
        Some(Commands::Update { revision, message }) => {
            app.cmd_update(&revision, message.as_deref()).await?
        }
        Some(Commands::Status) | None => app.cmd_status().await?,
    }
...
mod tests {
    ...
    async fn test_cmd_create_creates_new_pr() {
        ...
        let result = app.cmd_create("@").await;
        assert!(result.is_ok());
    ...
    async fn test_cmd_update_updates_existing_pr() {
        ...
        let result = app.cmd_update("@", Some("Update from review")).await;
        assert!(result.is_ok());
    ...
    async fn test_cmd_status_branch_exists_up_to_date() {
        ...
        let result = app.cmd_status().await;
        assert!(result.is_ok());
    }
    ...
    async fn test_cmd_update_errors_when_pr_is_closed() {
        ...
        let result = app.cmd_update("@", Some("Update after review")).await;
        assert!(result.is_err());
    ...
    async fn test_cmd_update_errors_when_parent_pr_is_outdated() {
        ...
        let result = app.cmd_update("@", Some("Update commit C")).await;
        assert!(result.is_err());
    ...
    async fn test_cmd_update_auto_restack_when_diffs_match() {
        ...
        let result = app.cmd_update("@", None).await;
        if let Err(e) = &result {
            eprintln!("Error: {}", e);
        }
        assert!(result.is_ok());
